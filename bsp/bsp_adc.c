...
void BSP_ADC_GPIO_Configure(ADC_TypeDef* ADCx, u32 chs)
{
	if (ADCx == ADC1) {
		...
		for (u8 i = 0; i < 8; i++) {
			if (chs & 1 << i)
				GPIO_Mode_IN_Init(GPIOA, (u16)(1 << i), GPIO_Mode_AIN ,NO_REMAP, NO_REMAP);	// PB0,PB1... be ignored
		}
	#endif
	}
	else {
		...
		for (u8 i = 0; i < 8; i++) {
			if (chs & 1 << i)
				GPIO_Mode_IN_Init(GPIOB, (u16)(1 << i), GPIO_Mode_AIN ,NO_REMAP, NO_REMAP);	// PB0,PB1... be ignored
		}
	#endif
		// Todo
	}
